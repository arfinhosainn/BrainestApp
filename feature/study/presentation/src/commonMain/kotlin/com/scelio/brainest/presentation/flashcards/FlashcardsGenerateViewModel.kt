...
data class FlashcardsGenerateState(
    ...
    val decks: List<FlashcardsDeckItem> = emptyList(),
...
class FlashcardsGenerateViewModel(
    ...
    fun loadDecks() {
        if (_state.value.isLoadingDecks) return

        _state.update { it.copy(isLoadingDecks = true, decksError = null) }

        viewModelScope.launch {
            val userId = awaitUserId()
            if (userId == null) {
                _state.update {
                    it.copy(isLoadingDecks = false, decksError = "No authenticated user found.")
                }
                return@launch
            }

            loadDecksForUser(userId)
        }
    }
    ...
    private suspend fun loadDecksForUser(userId: String) {
        val decksResult = repository.listDecks(userId)
        if (decksResult is Result.Failure) {
            _state.update {
                it.copy(
                    isLoadingDecks = false,
                    decksError = "Failed to load flashcards: ${decksResult.error}"
                )
            }
            return
        }

        val sessionsResult = repository.listStudySessions(userId)
        val sessions = when (sessionsResult) {
            is Result.Success -> sessionsResult.data
            is Result.Failure -> emptyList()
        }

        val totalSwipedByDeck = sessions
            .groupBy { it.deckId }
            .mapValues { entry -> entry.value.sumOf { it.totalSwiped } }
        ...
        val items = decks.map { deck ->
            FlashcardsDeckItem(
                ...
                totalSwiped = totalSwipedByDeck[deck.id] ?: 0
            )
        }
        ...
        _state.update {
            it.copy(
                isLoadingDecks = false,
                decks = items,
                decksError = if (sessionsResult is Result.Failure) {
                    "Some stats may be missing: ${sessionsResult.error}"
                } else {
                    null
                }
            )
